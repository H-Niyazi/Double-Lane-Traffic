...
int cars_num = 4000;             // total number of the cars
...
void lane_change(car* cars)
{
  ...
  // There might be a slight data dependence here when parallelizing as it is
  // also mentioned in the paper claiming that parallelization might give
  // slightly different results compared to the non-parallel simulation, but
  // because the if statement inside equals to a true condition very rarely and
  // even at those times it is almost impossible that we encounter a race
  // condition for the threads executing them, we can say that alomost always we
  // can assume our parallelization of this part to be data independent.
  #pragma omp parallel for
  for (int i = 0; i < cars_num; i++)
  {
